...
// Pure component for Plot Tooltip to prevent parent re-renders
const PlotTooltip = React.memo(({ plot, position }) => {
  if (!plot) return null;
  return (
    <div
...
// Pure component for Selected Plot Panel
  ...
  return (
    <div className="absolute top-4 right-4 z-40 bg-white rounded-2xl shadow-2xl p-4 sm:p-6 border border-gray-100 max-w-[280px] sm:max-w-sm w-full animate-in fade-in slide-in-from-right-8 duration-300">
      ...
      <div className="mb-4 flex gap-2">
        ...
        <span className="bg-teal-50 text-teal-800 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider border border-teal-100">
          {plot.facing}
        </span>
      </div>
...
SelectedPlotPanel.displayName = "SelectedPlotPanel";
...
export default function VentureMap() {
  ...
  // Optimization: Pre-calculate lookup map for O(1) plot lookups
  const plotLookup = useMemo(() => {
    const map = new Map();
    plotData.forEach(plot => {
      map.set(String(plot.number), plot);
    });
    return map;
  }, []);

  useEffect(() => {
    // AbortController to prevent memory leaks/race conditions on fast navigation
    const controller = new AbortController();

    fetch("/PYALAKURTI%20%20VENTURE%20LAYOUT.svg", { signal: controller.signal })
      .then((res) => res.text())
      .then((text) => setSvgContent(text))
      .catch((err) => {
        if (err.name !== 'AbortError') console.error("Error loading SVG:", err);
      });
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    if (target.tagName === "path") {
      ...
      if (id?.startsWith("plot-")) {
        const plotNumber = id.replace("plot-", "");
        const plot = plotLookup.get(plotNumber);
  ...
  return (
    <div className="relative w-full h-[400px] md:h-[600px] border border-gray-200 rounded-2xl overflow-hidden bg-[#fafafa] flex flex-col p-4 shadow-sm">
          ...
          {({ zoomIn, zoomOut, resetTransform }) => (
            <>
              ...
              <div className="absolute top-6 left-6 z-30 flex flex-col gap-2 bg-white/90 backdrop-blur-md p-2 rounded-xl shadow-lg border border-gray-100">
                <ControlBtn onClick={() => zoomIn()} title="Zoom In" icon={<PlusIcon />} />
                <ControlBtn onClick={() => zoomOut()} title="Zoom Out" icon={<MinusIcon />} />
                <ControlBtn onClick={() => resetTransform()} title="Reset Map" icon={<HomeIcon />} className="mt-2" />
              </div>
      ...
      <SelectedPlotPanel plot={selectedPlot} onClose={handleClosePopup} />
...
// Icon Components & Sub-components moved out of render
const ControlBtn = React.memo(({ onClick, title, icon, className = "" }) => (
  <button
    className={`w-10 h-10 flex items-center justify-center bg-white hover:bg-gray-50 text-[#1B4332] rounded-lg shadow-sm border border-gray-100 transition-colors ${className}`}
    onClick={onClick}
    title={title}
  >
    {icon}
...
ControlBtn.displayName = "ControlBtn";
...
const MapLoader = () => (
  <div className="flex-1 flex flex-col items-center justify-center text-gray-400 animate-pulse">
    <svg className="w-12 h-12 mb-4 animate-spin text-[#1B4332]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
    </svg>
    <p className="font-inter font-medium text-sm">Loading Interactive Concept Map...</p>
  </div>
);
